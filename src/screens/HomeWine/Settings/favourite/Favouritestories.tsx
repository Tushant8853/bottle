...
const Favouritestories: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFavoriteStories = async () => {
      try {
        ...
        // Fetch story details for favorite stories
        ...
        // Set favorite stories list
        setStoriesList(updatedStories);
        setSavedStatus(new Array(updatedStories.length).fill(false));  // Now set savedStatus as false because we are showing favorites only
  
        // Set favorite status for these stories
        ...
        console.error("Error in fetchFavoriteStories:", error);
      } finally {
        setIsLoading(false);
      }
    };
