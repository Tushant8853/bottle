...
const CameraInputModal: React.FC<Props> = ({
  visible,
  onClose,
  onRetake,
  photoUri,
  ...
  const handleDoThisLater = async (
    ...
    try {
      ...
      const { data: memoriesData, error } = await supabase
        ...
        .select("created_at, location_lat, location_long, id")
        .eq("user_id", UID);
      ...
      for (const memory of memoriesData) {
        ...
        const timeDifference =
          (currentTime.getTime() - memoryTime.getTime()) / (1000 * 60 * 60);

        if (timeDifference < 3 && distance <= 100) {
          // Update existing memory gallery
          ...
          const fileName = savedFilePath.substring(
          ...
          await savePendingTask(pendingTask);
          break;
        }
      }
      ...
      if (!isHandled) {
        ...
        if (memoryError) {
          console.error("Error saving new memory:", memoryError);
          setLoading(false);
          return;
        }
        ...
        if (galleryError) {
          console.error("Error saving to memory gallery:", galleryError);
          setLoading(false);
          return;
        }
      ...
      // Save pending task to AsyncStorage
     

      setLoading(false);
    } catch (error) {
      console.error('Failed to handle "Do this later":', error);
      setLoading(false);
    }
  };
  ...
  const savePendingTask = async (task: {
    ...
    input2: string;
    input3: string;
    memoryId?: string; // Optional if not found
}) => {
    try {
        // Retrieve the existing tasks from AsyncStorage
        const existingTasks = await AsyncStorage.getItem('PENDING_TASKS');
        ...
        await AsyncStorage.setItem('PENDING_TASKS', JSON.stringify(tasks));
  ...
  const handleSaveWine = async (
    ...
    const { data: bottleshock_memories, error: bottleshock_memoriesError } =
      await supabase
        .from("bottleshock_memories")
        .insert([
          {
    ...
    if (bottleshock_memoriesError) {
      console.error(
        "Error saving data to bottleshock_memory_gallery:",
        bottleshock_memoriesError
      );
    }
    ...
    if (memoryWinesError) {
      console.error(
        "Error saving data to bottleshock_memory_wines:",
        memoryWinesError
      );
      return;
    }
    ...
    const {
      data: bottleshock_memory_gallery,
      error: bottleshock_memory_galleryError,
    } = await supabase
      .from("bottleshock_memory_gallery")
      .insert([
        {
    ...
    if (bottleshock_memory_galleryError) {
      console.error(
        "Error saving data to bottleshock_memory_gallery:",
        bottleshock_memory_galleryError
      );
    }
  };
  const handleSameSaveWine = async (
    input1: string,
    input2: string,
    input3: string,
    SameId: number
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.inputModal}>
            ...
            <TextInput
              style={[styles.input, error1 ? styles.inputError : null]}
            ...
            />
            <TextInput
              style={[styles.input, error2 ? styles.inputError : null]}
            ...
            />
            <TextInput
              style={[styles.input, error3 ? styles.inputError : null]}
            ...
            <View style={styles.iosButtonGroup}>
              ...
              <Pressable
                style={[styles.iosButton, styles.iosDefaultButton]}
                onPress={async () => {
                  await handleDoThisLater(input1, input2, input3, photoUri);
                  onClose();
                  setDoneModalVisible(true);
                }}
              >
